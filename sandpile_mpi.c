...
void write_png(const char *filename, int **grid, int N, int M) {
    ...
    for (int y = 0; y < N; y++) {
        for (int x = 0; x < M; x++) {
            int val = grid[y][x];
            switch (val) {
                case 0: row[3 * x + 0] = 0;   row[3 * x + 1] = 0;   row[3 * x + 2] = 0;   break;
                case 1: row[3 * x + 0] = 0;   row[3 * x + 1] = 255; row[3 * x + 2] = 0;   break;
                case 2: row[3 * x + 0] = 0;   row[3 * x + 1] = 0;   row[3 * x + 2] = 255; break;
                case 3: row[3 * x + 0] = 255; row[3 * x + 1] = 0;   row[3 * x + 2] = 0;   break;
                default: row[3 * x + 0] = 255; row[3 * x + 1] = 255; row[3 * x + 2] = 255; break;
            }
        }
    ...
    while (changed) {
        ...
        for (int i = 0; i < N; i++) {
            for (int j = 0; j < M; j++) {
                ...
                if (distribute > 0) {
                    changed = true;
                    if (i > 0)     next[i-1][j] += distribute;
                    if (i < N-1)   next[i+1][j] += distribute;
                    if (j > 0)     next[i][j-1] += distribute;
                    if (j < M-1)   next[i][j+1] += distribute;
                }
            }
        }
